...
// Custom tooltip for temp chart — uses dynamic targetTemp from slider
...
export default function App() {
  ...
  // ── WebSocket: live agent command from Python backend ──────────────────────
    ...
    const connect = () => {
      ...
      ws.onmessage = (evt) => {
        try {
          ...
          // Override outdoor conditions with real data from Python scraper
  ...
  // FIX Bug 4: Dynamic decisions that update every tick based on real state
  ...
  // FIX: memoize triggerNextTick to avoid stale closures
  const triggerNextTick = useCallback(() => {
    ...
    setData((prev) => {
      ...
      // FIX Bug 5: simulate energy toward targetEnergy (not just random one-way decay)
      ...
      // FIX Bug 4: compute new decision based on actual gap
  ...
  // FIX Bug 5+7: CSV export with correct columns including indoor_temp and correct gap
  const handleExport = () => {
    const headerRow1 = [
      'timestamp',
      'temp_controls,,,',
    ...
    const headerRow2 = [
      'time',
      'indoor_temp', 'outdoor_temp', 'target_temp', 'temp_gap',
      'current_energy_w', 'target_energy_w', 'energy_gap_w',
      'outdoor_temp_c', 'humidity_pct', 'wind_kmh', 'wind_dir', 'aqi', 'rain_mm'
    ].join(',');
    const rows = data.map(row => {
      // FIX Bug 5: temp_gap = indoor - target (not outdoor - target)
      const tGap = (row.temp - targetTemp).toFixed(2);
      const eGap = (row.energy - targetEnergy).toFixed(0);
      return [
        row.time,
        row.temp, OUTDOOR_TEMP, targetTemp, tGap,
        row.energy, targetEnergy, eGap,
        OUTDOOR_TEMP, row.humidity, row.windSpeed, windDir, row.airQuality.toFixed(0), row.rain
      ].join(',');
    });
  ...
  // FIX Bug 2: "Gap to Target" now shows |indoor_temp - target|, NOT outdoor - target
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${bgMain} p-4 md:p-6 font-sans`}>
      {/* FIX Bug 6: Global slider styles — moved outside the card */}
      <style>{`
        ...
        input[type='range'].energy-slider::-webkit-slider-thumb {
          background: #10b981;
          box-shadow: 0 0 0 3px #10b981;
        }
        input[type='range'].energy-slider::-moz-range-thumb {
          background: #10b981;
          box-shadow: 0 0 0 3px #10b981;
        }
      `}</style>
      ...
      <div className="max-w-7xl mx-auto flex flex-col gap-5">
        ...
        <header className={`flex flex-col md:flex-row justify-between items-start md:items-center gap-3 border-b pb-5 ${isDark ? 'border-gray-700/50' : 'border-gray-300'}`}>
          ...
          <div className="flex flex-wrap gap-2 items-center">
            ...
            <button
              ...
              className={`p-2 rounded-md border ${cardBg} hover:opacity-80 transition-opacity`}
              title="Toggle Theme"
            >
            ...
            <button
              ...
              className="flex items-center gap-2 px-3 py-2 text-xs font-medium rounded-md bg-emerald-600 text-white hover:bg-emerald-700 transition-colors border border-emerald-500"
            >
        ...
        {/* ── Row 1: Temperature Controls + Live Gauge ── */}
          ...
          <div className={`col-span-1 lg:col-span-2 rounded-xl border ${cardBg} p-5`}>
            ...
            <div className="grid grid-cols-3 gap-3 mb-4">
              ...
              <div className={`p-3 rounded-lg border ${statCell}`}>
                ...
                {/* FIX Bug 2: Shows indoor-to-target gap, sign indicates over/under */}
            ...
            <div className="w-full">
              ...
              <div className="relative w-full h-5 flex items-center">
                ...
                <div
                  ...
                  style={{ width: `${((targetTemp - 18) / (30 - 18)) * 100}%` }}
                />
                <input
                  type="range" min="18" max="30" step="1"
                  value={targetTemp}
                  onChange={(e) => setTargetTemp(Number(e.target.value))}
                  className="absolute w-full h-1.5 rounded-full appearance-none cursor-pointer bg-transparent"
            ...
            <div className="relative w-36 h-36 mt-6">
              <svg className="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
                ...
                <circle
                  ...
                  stroke={latestData?.temp > targetTemp + 2 ? '#f59e0b' : '#10b981'}
                  strokeWidth="7"
                  strokeDasharray={`${(Math.max(0, Math.min(latestData?.temp ?? 22, 40)) / 40) * 276} 276`}
        ...
        {/* ── Row 2: Energy Consumption Control (mirrors Temp Controls exactly) ── */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          <div className={`col-span-1 lg:col-span-2 rounded-xl border ${cardBg} p-5`}>
            <h2 className={`text-xs font-semibold uppercase tracking-wider flex items-center gap-2 mb-4 ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
            ...
            <div className="w-full">
              ...
              <div className="relative w-full h-5 flex items-center">
                ...
                <div
                  ...
                  style={{ width: `${((targetEnergy - 300) / (1500 - 300)) * 100}%` }}
                />
                <input
                  type="range" min="300" max="1500" step="50"
                  value={targetEnergy}
                  onChange={(e) => setTargetEnergy(Number(e.target.value))}
                  className="energy-slider absolute w-full h-1.5 rounded-full appearance-none cursor-pointer bg-transparent"
                  style={{ WebkitAppearance: 'none' }}
                />
              </div>
            </div>
          ...
          {/* Energy status indicator — mirrors the gauge position */}
          <div className={`col-span-1 rounded-xl border ${cardBg} p-5 flex flex-col items-center justify-center relative`}>
            <h2 className={`absolute top-5 left-5 text-xs font-semibold uppercase tracking-wider flex items-center gap-2 ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
            ...
            <div className="relative w-36 h-36 mt-6">
              <svg className="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
                ...
                <circle
                  cx="50" cy="50" r="44" fill="none"
                  stroke={currentEnergy > targetEnergy ? '#f59e0b' : '#10b981'}
                  strokeWidth="7"
                  strokeDasharray={`${(Math.min(currentEnergy, 1500) / 1500) * 276} 276`}
                  className="transition-all duration-1000 ease-out"
                />
              </svg>
              <div className="absolute inset-0 flex flex-col items-center justify-center">
        ...
        {/* ── Row 3: Outdoor Conditions (6 params) ── */}
        <div className={`rounded-xl border ${cardBg} p-5`}>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
            ...
            <div className={`rounded-lg border p-3 ${statCell}`}>
              <div className={`text-[10px] uppercase font-semibold tracking-wide ${textMuted} flex items-center gap-1 mb-2`}>
                <Thermometer size={11} className="text-orange-400" /> Outdoor
              </div>
            ...
            <div className={`rounded-lg border p-3 ${statCell}`}>
              <div className={`text-[10px] uppercase font-semibold tracking-wide ${textMuted} flex items-center gap-1 mb-2`}>
                <Droplets size={11} className="text-blue-400" /> Humidity
              </div>
            ...
            <div className={`rounded-lg border p-3 ${statCell}`}>
              <div className={`text-[10px] uppercase font-semibold tracking-wide ${textMuted} flex items-center gap-1 mb-2`}>
                <Wind size={11} className="text-sky-400" /> Wind
              </div>
            ...
            <div className={`rounded-lg border p-3 ${statCell}`}>
              <div className={`text-[10px] uppercase font-semibold tracking-wide ${textMuted} flex items-center gap-1 mb-2`}>
                <Eye size={11} className="text-purple-400" /> Air Quality
              </div>
            ...
            <div className={`rounded-lg border p-3 ${statCell}`}>
              <div className={`text-[10px] uppercase font-semibold tracking-wide ${textMuted} flex items-center gap-1 mb-2`}>
                <CloudRain size={11} className="text-indigo-400" /> Rainfall
              </div>
            ...
            <div className={`rounded-lg border p-3 ${statCell}`}>
              <div className={`text-[10px] uppercase font-semibold tracking-wide ${textMuted} flex items-center gap-1 mb-2`}>
                <Droplets size={11} className="text-teal-400" /> Moisture
              </div>
              <div className={`text-base font-bold ${outdoorRain > 1 ? 'text-indigo-400' : outdoorHumidity > 60 ? 'text-amber-400' : 'text-emerald-400'}`}>
                {moistureCondition}
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          <div className={`rounded-xl border ${cardBg} p-5 flex flex-col`}>
            ...
            <div className={`flex-1 rounded-lg p-4 border ${cardInner} flex flex-col justify-center gap-2`}>
              ...
              <div className={`text-sm italic leading-relaxed ${textMuted}`}>
                &ldquo;{aiThought.thought}&rdquo;
              </div>
            </div>
          ...
          {/* Last 5 Decisions — FIX Bug 4: now dynamic */}
          <div className={`rounded-xl border ${cardBg} p-5 flex flex-col`}>
            <h2 className={`text-xs font-semibold uppercase tracking-wider mb-3 flex items-center gap-2 ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
            ...
            <div className="flex flex-col gap-0">
              {decisions.map((d, i) => (
                <div
                  key={i}
                  className={`flex items-center justify-between py-2.5 ${i < decisions.length - 1 ? `border-b ${isDark ? 'border-gray-800' : 'border-gray-200'}` : ''}`}
                >
                  <div className="flex items-center gap-3">
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold border ${d.good
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          {/* Temperature Chart — FIX Bug 1+8: uses targetTemp not IDEAL_TEMP */}
          <div className={`rounded-xl border ${cardBg} p-4`}>
            <div className="flex justify-between items-center mb-2">
              <h3 className={`text-xs font-semibold uppercase tracking-wider ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
                Temperature Over Time
              </h3>
              <span className={`text-[10px] ${textMuted} flex items-center gap-1.5`}>
                <span className="inline-block w-5 border-t-2 border-dashed border-emerald-500"></span>
                Ideal {targetTemp}°C
              </span>
            </div>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={data} margin={{ top: 8, right: 12, left: -18, bottom: 0 }}>
                  ...
                  <Area
                    ...
                    fillOpacity={1} fill="url(#tempGrad)"
          ...
          <div className={`rounded-xl border ${cardBg} p-4`}>
            <div className="flex justify-between items-center mb-2">
              ...
              <span className={`text-[10px] font-mono px-2 py-0.5 rounded-md ${isDark ? 'bg-emerald-500/15 text-emerald-400' : 'bg-emerald-100 text-emerald-700'}`}>
                −12% Cost Today
              </span>
            </div>
            <div style={{ width: '100%', height: 160 }}>
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={data} margin={{ top: 8, right: 12, left: -18, bottom: 0 }}>
                  ...
                  <Area
                    ...
                    fillOpacity={1} fill="url(#energyGrad)"
                    isAnimationActive={false}
                  />
                </AreaChart>
              </ResponsiveContainer>
        ...
        <div className={`rounded-xl border overflow-hidden font-mono ${isDark ? 'border-gray-800 bg-[#0d1117]' : 'border-gray-700 bg-[#0d1117]'}`}>
          ...
          <div className="flex items-center justify-between px-5 py-3 border-b border-gray-800 bg-black/30">
            <span className="text-[10px] text-gray-500 uppercase tracking-widest">
              {'>_ AI_AGENT_STREAMSOCKET // LISTENING'}
            </span>
            <div className="flex items-center gap-4">
              <span className="text-xs text-gray-400">
                Next payload in <strong className="text-emerald-400">{countdown}s</strong>
              </span>
...
{`{
  ...
  "status": "${currentGapVal > 2 ? 'WARN_THERMAL_RISK' : 'OK'}"
            ...
            {/* Right: live agent command from Python backend */}
  ...
  "avg_r10":   ${agentCommand.avg_reward_10.toFixed(3)}
                ...
                <div className="flex-1 flex flex-col items-center justify-center gap-3 py-4">
                  ...
                  <div className="text-xs text-gray-400 text-center leading-relaxed">
                    ...
                    <span className="text-[10px] text-gray-600">run: python backend/main.py</span>
                  </div>
                </div>
              )}
            </div>
